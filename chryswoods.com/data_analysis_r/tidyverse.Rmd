...
1. No, the month in which the observation was taken is used as a column header, 
   and is not in its own column. 
2. No, a row contains 12 observations across a year, plus an average of the rows
3. No, the month value for January 1659 is a column header and does not have its
   own cell. And the YEAR column refers to an average that is a derived value
   from the observations of the other cells (it is not really a "value" of the data)
...
should be removed as it is not an observation. We can do this using;
...
The final problem to solve is that the "month" is just a character string. 
...
which is included in base R. The general syntax is;

```R
factor(data, levels)
```

would return the passed `data` factored into one of the levels specified
in `levels`.

We can now edit our data to translate each month string into a
month factor. We do this using the [mutate](https://dplyr.tidyverse.org/reference/mutate.html)
function. This function can edit a tibble, e.g.

```R
mutate(temperature=temperature+5)
```

would edit the "temperature" column, and add "5" to every single value 
in this column. In our case, we want to factor each value, so we
need 
...
Putting this together with what we have before, we now get;
...
## [Previous](files.html) | [Next](mapping.html)
