...
(also called an interpreted language), which is much easier to write
than when using a compiled language, such as C, C++ or Fortran.

Computer processors only understand (and can execute) machine code.
All programs, regardless of language, have to be converted to
machine code to be executed on your computer.
...
The Python Virtual Machine executes that bytecode, converting it into
...
can be very slow. This is a problem for two reasons:

1. Who wants to wait longer than needed for your calculation to complete?
2. Executing bytecode is more complex for your processor, so not only is
   it taking longer to complete, it may be consuming more electrical
   energy while it is doing so. Programs written in Python can consume
   [significantly more electrical energy](https://greenlab.di.uminho.pt/wp-content/uploads/2017/10/sleFinal.pdf)
   than programs written in compiled languages
